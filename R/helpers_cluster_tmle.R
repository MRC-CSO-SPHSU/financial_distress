...
doTMLE<- function(train, Qinit.Indv=F, QAdj,  work.model, gAdj=NULL, prob.txt,
	...
	if( is.null(gAdj) ){	
		...
		if(Do.SuperLearner){
			...
			p.out <- SuperLearner::SuperLearner(Y=A.train, X=as.data.frame(gAdj.train),  family="binomial",
...
do.Init.Qbar.SL<- function(train, QAdj, SL.library, verbose=F){
	...
	outcome.SL<- SuperLearner::SuperLearner(Y=train$Y, X=train.temp, family='binomial', SL.library= SL.library, 
